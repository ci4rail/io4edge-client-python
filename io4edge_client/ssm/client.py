...
class Client(ClientConnection):
    ...
    def __init__(
        self, addr: str, command_timeout: int = 5, connect: bool = False
    ) -> None:
        self._logger = io4edge_client_logger("ssm.Client")
        self._logger.debug("Initializing SSM client")
        fb_client = FbClient(
            "_io4edge_ssm._tcp", addr, command_timeout, connect=connect
        )
        ...
        if hasattr(fs_response, "state_error"):
            ...
            match rsp:
                ...
                case Pb.StateCommandResponseType.INVALID_STATE_ERROR:
                    raise InvalidStateError("Invalid state for setting error: ", msg)
        ...
        if hasattr(fs_response, "state_error"):
            ...
            match rsp:
                case Pb.StateCommandResponseType.STATE_OK:
                    return
                case Pb.StateCommandResponseType.INVALID_STATE_ERROR:
                    raise InvalidStateError("Invalid state for resolving error: ", msg)
                case Pb.StateCommandResponseType.UNKNOWN_STATE_ERROR:
                    raise UnknownError("Unknown error while resolving error: ", msg)
                case _:
                    raise RuntimeError("Unhandled response from SSM functionblock")

    @connectable
    def off(self, msg: str) -> None:
        """
        Indicate off state
        ...
        @raises ConnectionError: if not connected to the device
        @raises InvalidStateError: if operation attempted in an invalid state
        @raises UnknownError: if unknown error occurs while signaling off
        """
        fs_cmd = Pb.FunctionControlSet()
        fs_cmd.off = msg
        fs_response = Pb.FunctionControlSetResponse()
        ...
        if hasattr(fs_response, "host_error"):
            ...
            match rsp:
                ...
                case Pb.HostCommandResponseType.INVALID_CMD_STATE_ERROR:
                    raise InvalidStateError("Invalid state for shutdown signal: ", msg)
                case Pb.HostCommandResponseType.UNKNOWN_CMD_ERROR:
                    raise UnknownError("Unknown error while signaling shutdown: ", msg)
                case _:
                    raise RuntimeError("Unhandled response from SSM functionblock")
        ...
        if hasattr(fs_response, "host_error"):
            ...
            match rsp:
                case Pb.HostCommandResponseType.CMD_OK:
                    return
                case Pb.HostCommandResponseType.INVALID_CMD_STATE_ERROR:
                    raise InvalidStateError("Invalid state for signaling reboot: ", msg)
                case Pb.HostCommandResponseType.UNKNOWN_CMD_ERROR:
                    raise UnknownError("Unknown error while signaling reboot: ", msg)
                case _:
                    raise RuntimeError("Unhandled response from SSM functionblock")
